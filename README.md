...
Musyko is a social music-listening app in progress. Phase 2 adds email/password authentication and basic user profiles while preserving the Audius player from Phase 1.
...
`AUDIUS_API_BASE_URL` defaults to `https://api.audius.co/v1` and `AUDIUS_APP_NAME` defaults to `musyko`. No service-role key is used or exposed.
...
- `GET /v1/tracks/trending?limit=10&app_name=musyko` for track metadata
- `GET /v1/tracks/{track_id}/stream?app_name=musyko` for the native audio stream
