...
export default function AuthForm({ mode }: AuthFormProps) {
  ...
  return <main className="flex min-h-screen items-center justify-center px-5 py-10">
    <section className="w-full max-w-[430px] rounded-[28px] border border-[#ded6ca] bg-[#fffdf9] p-6 shadow-[0_24px_70px_rgba(80,60,35,0.11)] sm:p-8">
      <Link href="/" className="font-display text-xl font-bold tracking-[-0.04em]">musyko<span className="text-[var(--orange)]">.</span></Link>
      <div className="mb-8 mt-10"><p className="mb-2 text-xs font-bold uppercase tracking-[0.2em] text-[var(--orange)]">{isSignup ? "New here" : "Welcome back"}</p><h1 className="font-display text-3xl font-bold tracking-[-0.05em]">{isSignup ? "Create your account" : "Log in to Musyko"}</h1><p className="mt-2 text-sm leading-6 text-[var(--ink-muted)]">{isSignup ? "Choose a username and make your listening space yours." : "Your player is waiting."}</p></div>
